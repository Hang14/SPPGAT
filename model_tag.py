...
class classifier(nn.Module):
    ...
    def forward(self, input_ids, labels = None):
        ...
        SPPGAT_embeddings = []   
        if (input_ids[6] != []) and (input_ids[7] != []):
        ...
        position_ids = torch.arange(self.name_length, dtype=torch.long, device= labels.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids[0])
        position_embeddings = self.position_embeddings(position_ids)
        if SPPGAT_embeddings == []:
            SPPGAT_embeddings = position_embeddings
        else:
        ...
        features_output = self.dropout(encoded_layers)

        # Task-specific
        logits = self.classifier(torch.reshape(features_output,(labels.shape[0],-1)))
